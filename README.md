...
# 创建带专属图片文件夹的文章
./scripts/new_post.sh agent "agent-framework-comparison" img
```

脚本支持 `llm`、`agent`、`evals` 三类。如果添加了 `img` 参数，脚本会在文章同级目录下创建一个名为 `{日期}-{slug}-img` 的文件夹，用于存放该文章的图片。
...
将图片放入 `assets/images/` 目录下（建议按日期或主题分子目录）。在 Markdown 中使用以下语法引用：

```md
![图片描述]({{ "/assets/images/your-image.png" | relative_url }})
```

### 方式二：同级放置（适合特定博客图片）

如果你希望将图片与博客 `.md` 文件放在同一个目录下，可以直接将图片放入对应的 collection 目录中（如 `_agent_posts/2026/05/`）。引用时需使用相对于根目录的路径：

```md
![图片描述]({{ "/_agent_posts/2026/05/your-image.png" | relative_url }})
```

> **注意**：系统已为所有文章内的图片自动添加了**居中显示、圆角边缘以及轻微阴影**，无需额外编写 HTML 标签。
