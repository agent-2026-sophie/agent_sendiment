...
# system prompt
Hermes Agent的system prompt核心内容如下，通用的身份定位让其能够做各种任务，可复用技能 & 强制skill机制相关内容实现了其可沉淀并使用技能，达到自进化的目标：
...
# 工具
Hermes Agent 的工具系统走的是自注册模式：每个工具在代码层面定义好后，会自动把自己注册到中央注册表里，供模型调用。这种自注册模式让用户自定义工具变得很容易，只需要定义python函数然后加上装饰器就是一个新的工具。tools/ 目录下共约 80 个 Python 文件、247 个函数实现——但这里需要澄清一个容易混淆的点：247 是底层函数的总数，实际暴露给模型调用的核心工具只有约 60-70 个。
这 247 个底层实现分为几个层次：
- 模型可见的核心工具（~60+）：如 read_file、web_search、browser_navigate、delegate_task 等，这些是模型在对话中实际能调用的
- 多后端适配层（~20+）：浏览器自动化支持 Browserbase、Browser Use、Firecrawl、Camofox 等多个云服务商，每个都有独立的连接、认证、重试逻辑
- 执行环境抽象（~30+）：local、docker、ssh、modal、daytona、singularity 六种运行环境的文件同步、命令执行、生命周期管理
- 工具内部辅助函数（~100+）：如浏览器会话管理、页面快照解析、元素定位、MCP 服务器的 OAuth 流程管理、工具结果压缩等
- 安全与校验层（~30+）：路径安全检查、URL 安全过滤、凭据文件白名单、危险命令审批、Prompt 注入检测等

Hermes Agent的工具调用相关内容有如下特点：
...
- 第一步：启动会话：在项目根目录启动 Hermes，系统加载根目录的 HERMES.md，了解通用项目信息。此时 Agent 还不知道后端和前端的特定规则。
- 第二步：探索后端目录：用户让 Agent "看看后端用户模型是怎么定义的"。Agent 调用工具读取backend/src/models/user.py。
- 第三步：路径提取与目录发现：系统从工具调用中提取到路径 backend/src/models/user.py，识别出这是一个新目录（backend/ 及其子目录）。系统检查这个目录是否已经被处理过——还没有。
- 第四步：向上扫描上下文文件：系统从 backend/src/models/ 开始向上扫描（最多5层），查找上下文文件。它发现 backend/HERMES.md 存在，而根目录的 HERMES.md 已经在启动时加载过了（去重机制确保不会重复加载）。
- 第五步：注入到工具结果：backend/HERMES.md 的内容被追加到当前工具调用的结果中，Agent 看到的返回内容变成：（有两类工具都会触发目录感知：1.文件操作工具：read_file（读文件）、write_file（写文件）、patch（编辑文件）等，这些工具有 path 或 file_path 参数；终端工具：terminal，从命令字符串中解析路径（比如你执行 cat backend/src/models.py，系统能识别出 backend/src/models.py 这个路径）
文件内容：user.py 的代码...]
...
- 第六步：模型获得上下文：Agent 在收到文件内容的同时，也看到了后端目录的特定规则。如果接下来你让 Agent "给这个模型写个测试"，它就能按照后端目录的约定来生成代码了。
...
# context压缩
Hermes的context压缩策略和claude code相比相对简单。Hermes 在当前消息总 token 数达到模型 context window 的 50% 时触发压缩。这是一个偏保守的阈值——其他框架通常等到 80-90% 才动手。选择 50% 的原因是：压缩本身需要调用一次辅助 LLM（如 Claude Haiku 或 Gemini Flash）来生成摘要。如果等到 context 几乎占满再压缩，一轮对话里可能连续发生"快满了→调用压缩→压缩完继续对话→又快满了"，延迟会显著增加。提前到 50% 压缩，可以让一次摘要覆盖接下来较长一段轮次。
压缩不是简单地把整段历史压缩，而是有选择地保留关键部分，把中间内容摘要化：
- 头部保护：最初的对话永远保留：最开始的 3 条消息（通常是系统背景、用户最初的任务描述）永远原样保留。这些信息定义了整个会话的基调，一旦丢失，模型可能忘记"我们在做什么"。
- 尾部保护：最近的对话完整保留：最近的几轮对话（大约占 context window 20% 的 token 预算）也原样保留。这确保模型清楚"刚才发生了什么"和"用户最新说了什么"。最后一条用户消息特别重要——压缩逻辑确保它永远在保留的尾部范围内。
- 中间部分：用 LLM 生成摘要头部和尾部之间的内容，交给辅助 LLM 生成摘要。摘要的长度大约是原内容的 20%——比如 10,000 token 的历史会被压缩成 2,000 token 的摘要。摘要的主要内容如下：
...
虽然没有 Plan 模式，Hermes 提供了 todo 工具来实现类似的规划功能，让模型自发的进行任务规划，也能具有分解复杂任务（把大目标拆成可执行的子任务）和跟踪进度（标记任务为 pending/in_progress/completed）的能力。下面是一个hermes agent调用todo工具的例子，todo工具的输入输出信息存在模型的congtext里边，就能让模型感知当前任务状态了。
用户：帮我实现一个用户认证系统

Agent：我来帮你规划这个任务，先创建一个任务列表。->调用todo工具
todo工具输入：
...
更加完善的memory机制
Hermes主要依赖于MEMORY和USER两种md文件用于记录用户的偏好，Hermes还专门为模型设计了“memory”工具让 Agent 把重要的用户信息写入持久化存储。记忆文件默认是加载到system prompt中的，本次会话的 system prompt 并不会改变。记忆内容要等到下次会话启动时才作为新快照读入。这种设计是为了保护 prefix cache。因此memory机制主要是为了跨session设计。
...
稳定事实 — 未来会话仍有用的信息
更完善的skill机制
 memory 用来存事实（用户喜欢什么），skill 用来存的是程序（怎么做某件事，具体操作流程）。除了SP中强调让模型遇到复杂情况沉淀skill，以及执行任务积极加载skill外，Hermes还为skills设计了单独工具，提高了skill在框架中的地位，让模型能够更好使用skill，有如下skill相关工具：
...
- skill_manage：创建/修改/删除技能，完成任务后保存经验,或修复过时技能。skill manage是多种能力的结合，比较复杂，可实现的能力具体如下：
Action
用途
关键参数
典型使用场景
...
添加模板、脚本、参考文档到 references//templates//scripts/
remove_file
删除辅助文件
name, file_path
清理无用的辅助文件
...
Memory 存结论，Skill 存方法，但真正的学习往往发生在试错过程中——那些失败的尝试、错误的假设、突然的顿悟，不会自动变成 Memory 或 Skill，却包含着宝贵的经验。Session Search 让 Agent 能回顾自己过去的完整思考轨迹（因为存在context压缩机制，历史轨迹并不会一直存在于context中），拿到具体处理问题的详细信息，而claude code等其他agent则仅仅是将历史轨迹落盘当作日志，不会再去reveiw。Hermes也专门设计了session_search工具，具体工作流程如下：
...
# Gateway 架构：多平台支持
hermes agent和claude code、codex等本地CLI不同，它是一个Agent 服务端，后台可以跑多个session服务（AIAgent实例）不同的用户。其实现了异步消息网关，支持 Telegram、Discord、Slack、WhatsApp 等 20+ 平台同时接入请求。其架构分层如下：
暂时无法在飞书文档外展示此内容
Gateway 的核心职责是消息路由和会话隔离，具体有如下功能：
- 接收消息：从多个平台（Telegram、Discord 等）接收用户消息
- 生成 Session Key：根据消息来源（私聊/群聊/线程（群聊内的子话题）、用户ID、聊天ID）生成唯一标识
- 路由到 AIAgent：根据 Session Key 找到或创建对应的 AIAgent 实例
- 隔离上下文：确保不同会话的上下文互不干扰

Gateway大致可分为如下两种模式：
- 私聊： Gateway 与单个用户的私信对话。例如用户在 Telegram 私聊中给 Bot 发消息，每个私聊对话有独立的 Session Key 和上下文。
- 群聊： Gateway 在群组/频道中与多个用户互动，可以配置为每个用户独立会话（默认），或群内共享会话。

hermes agent支持同时跑多个session，但也不能无限制的增加，有一定的淘汰机制。比如超过 128 个活跃会话时，淘汰最近最少使用；1 小时无消息的会话，清理对应 AIAgent 释放内存。